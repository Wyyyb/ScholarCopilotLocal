...
def monitor_output(process):
    ...
    while True:
        try:
            ...
            if not line and not is_process_running(process):
                break
            ...
            if line:
                print(f"[Gradio Output] {line}")
        except UnicodeDecodeError:
            print("解码输出时发生错误，跳过此行")
            continue
        except:
            break
...
def get_gradio_url(process):
    """从程序输出中提取gradio URL"""
    url = None
    while url is None and is_process_running(process):
        try:
            line = process.stdout.readline()
            if not line:
                continue
    ...
    while True:
        try:
            ...
            if process is None or not is_process_running(process):
                ...
                # 启动输出监控线程
                output_thread = threading.Thread(target=monitor_output, args=(process,))
